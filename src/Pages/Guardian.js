...
import GuardianCreate from "../Components/modals/GuardianCreate";
import axios from "axios";
import GuardianUpdate from "../Components/modals/GuardianUpdate";
...
const Guardian = () => {
  ...
  const { data, isError, isFetching, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      const fetchURL = new URL(`${process.env.REACT_APP_BASE_URL}/guardians`);
  ...
  const deletePost = useMutation((id) => {
    return axios
      .delete(`${process.env.REACT_APP_BASE_URL}/guardian/${id}`)
      .then(() => {
  ...
  return (
    <section className="bg-white h-full w-full  p-4">
      <h1 className="mb-4 font-semibold tracking-wide text-lg">Guardians</h1>
